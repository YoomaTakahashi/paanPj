                ...
                <v-table>
                    ...
                    <tbody>
                        <tr v-for="(item,index) in result" :key="item.id_eva">
                            ...
                            <td class="text-center border">รอบการประเมิน{{ items.round_sys }} ปี {{ year_sys }}</td>
                            <td class="text-center border">{{ items.status_eva === 1 ? 'ยังไม่ได้ประเมิน' : items.status_commit === 'y' ? ประเมินสำเร็จ : item.status_eva ===2 ? 'รอกรรมการประเมิน':'ประเมินเสร็จสิ้น' }}</td>
                            <td class="text-center border">
                                <v-btn color="blue" class="text-white" size="small" @click="go(item.id_eva)">ประเมิน</v-btn>
                            </td>
                        </tr>
